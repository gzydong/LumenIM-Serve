...
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='用户聊天记录表';

CREATE TABLE `lar_chat_records_files` (
  ...
  `save_type` tinyint(3) unsigned DEFAULT '0' COMMENT '文件保存方式（0:本地 1:第三方[阿里OOS、七牛云] ）',
  ...
  `created_at` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='用户聊天记录（文件消息）';

CREATE TABLE `lar_chat_records_forward` (
...
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='用户聊天记录（会话记录转发消息）';

CREATE TABLE `lar_chat_records_code` (
...
CREATE TABLE `lar_chat_records_invite` (
...
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='用户聊天记录（入群/退群消息）';
