...
class ArticleLogic extends Logic
{
    ...
    {
        $info = Article::where('id', $article_id)->where('user_id', $uid)->first(['id', 'class_id', 'tags_id', 'title', 'status', 'abstract', 'is_asterisk','created_at' ,'updated_at']);
        if (!$info) return [];
